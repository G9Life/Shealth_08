...
| **세부 문서** | [02-01 네이밍](./02-01단계%20—%20네이밍%20개선%20리팩토링.md) · [02-02 하드코드](./02-02단계%20—%20하드코드%20및%20전역변수%20제거%20리팩토링.md) · [02-03 함수 추출](./02-03단계%20—%20함수%20추출%20리팩토링.md) · [02-04 중복 제거](./02-04단계%20—%20반복중복%20제거%20리팩토링.md) |
| **목적** | 네이밍 개선, 하드코드·전역 제거, 함수 추출, 반복/중복 제거, 입력 검증 반영 |
...
| 연령대 판별 | 루프 내 `(age/10)*10` 등 | `belongsToAgeDecade(age, ageDecade)` |
| 연령대 통계 | `if (a == 20) ... else if (a == 70)` ×6 | `computeDistributionForDecade()` + map |
| 비율 조회 | `getBmiRatio` 24분기 `if-else` | `getCategoryPercent()` + `percentForCategory()` |
| 연령대 순회 | `for (20; <=70; +=10)` 다수 반복 | `forEachAgeDecade()` |
| 진입 API | `calculateBmi()` | `loadAndCalculate()` |
| `main` | 6회 `printf` 중복, 매직 넘버 | `forEachAgeDecade` + `printAgeDecadeDistribution()` |
...
|------|-------------|------------|------|
| `SHealth.h` | 28 | 79 | enum·상수·템플릿 헬퍼 |
| `SHealth.cpp` | 148 | 259 | 파이프라인·집계 헬퍼 |
| `SHealthBMI.cpp` | 28 | 33 | 경로·출력만 담당 |
| `SHealthBMITest.cpp` | 6 | 71 | TC 7건 + Fixture |
...
| `getBmiRatio(...)` | `getBmiRatio(...)` (별칭 유지) | `getCategoryPercent` 위임 |
...
### 4.2 고정 배열 → `std::vector<HealthRecord>`

**Before**: `int ages[10000]`, `double weights[10000]` 등.

**After**: RAII·STL 활용, 레코드 수 제한 없음.

### 4.3 24개 통계 멤버 → `unordered_map`
...
| Before | After |
|--------|-------|
| `getBmiRatio(20, 100)` 등 24회 인자 | `getCategoryPercent(ageDecade, BmiCategory::…)` |
| `for (ageDecade = 20; …)` | `SHealth::forEachAgeDecade` |
| `"shealth.dat"` 고정 | `kDefaultDataFile` + `SHealth::resolveDataFilePath()` + CLI |
...
| `resolveDataFilePath` | 프로젝트 루트·`build/` 양쪽에서 데이터 파일 탐색 |
| `sumCategoryPercents` | 연령대별 4범주 비율 합 |

### 5.2 `main` (`SHealthBMI.cpp`)

| 함수 | 역할 |
|------|------|
| `printAgeDecadeDistribution()` | 연령대별 4범주 비율 `printf` 출력 |
| `main` | 경로 해석 → `loadAndCalculate` → `forEachAgeDecade` 출력 |

---

## 6. 중복 제거

### 6.1 연령대 루프 — `forEachAgeDecade`

체중 보정·통계 집계·`main` 출력·테스트가 **동일 템플릿**을 사용한다.

```cpp
template <typename Callback>
static void forEachAgeDecade(Callback&& callback) {
    for (int ageDecade = kMinAgeDecade; ageDecade <= kMaxAgeDecade;
         ageDecade += kAgeDecadeSpan) {
        callback(ageDecade);
    }
}
```
...
| Before | After |
|--------|-------|
| `CategoryCounts` 4개 `int` 멤버 | `std::array<int, 4> byCategory` |
| `incrementCategoryCount` `switch` 4갈래 | `bmiCategoryIndex()` + 배열 인덱스 |
| `toPercentDistribution` 4줄 수동 대입 | `percentFields[]` 루프 |
| `getCategoryPercent` 내 `switch` | `percentForCategory()` 헬퍼 |

### 6.3 테스트 중복 제거

- `SHealthLoadedDataFixture`: `resolveDataFilePath` + `loadAndCalculate` 1회
- `sumCategoryPercents`: 비율 합 ≈ 100% 검증 단순화
...
README 규칙 `20 ≤ age < 30`과 일치하도록 **`belongsToAgeDecade()`** 도입 (`(age/10)*10` 오류 제거).
...
| `SHealthLoadedDataFixture.LoadSampleDataFile` | `shealth.dat` 로드 스모크 |
| `SHealthLoadedDataFixture.AgeDecadeDistributionSumsToOneHundred` | `sumCategoryPercents` ≈ 100% |
| `SHealthLoadedDataFixture.GetCategoryPercentAcceptsLegacyCategoryCode` | enum / int(200) / `getBmiRatio` |
...
| 파일 | 변경 유형 |
|------|-----------|
| `src/main/cpp/SHealth.h` | 구조·API·상수·템플릿·컨테이너 |
| `src/main/cpp/SHealth.cpp` | 전면 리팩토링·집계 헬퍼 |
| `src/main/cpp/SHealthBMI.cpp` | 출력·CLI·`forEachAgeDecade` |
| `src/test/cpp/SHealthBMITest.cpp` | Fixture·TC 7건 |
...
| 유지보수 | 연령대 추가 시 멤버·분기 다수 | map·`forEachAgeDecade`로 확장 용이 |
| 안전성 | 배열 오버플로우 위험 | `vector`, 검증 함수 |
| 테스트 용이성 | `FAIL()` only | 정적 메서드·Fixture |
| DRY | 연령대·카테고리·경로·로드 중복 | 공통 API로 통합 |
...
1차 리팩토링으로 **매직 넘버, 24개 통계 멤버, C 고정 배열, God Method, main·테스트 중복** 등 의도된 코드 스멜을 제거하고, **모던 C++(STL, `enum class`, `constexpr`, 구조체, 템플릿 콜백)** 스타일로 정리하였다.

- README 도메인: **BMI 25.0 비만**, **연령대 10년 구간**, **입력 검증(age > 0)** 반영
- `getCategoryPercent` / `getBmiRatio` API로 enum·레거시 `100~400` 코드 모두 지원
- **7개 단위 테스트** 전부 통과, `shealth.dat` 기준 실행 결과 확인
...
| 연령대·카테고리 반복 | ✅ `forEachAgeDecade`, 배열 헬퍼 |
| 네이밍 | ✅ |
| BMI 25 버그 | ✅ |
| 테스트 부재 | ✅ 기초 7건 (확충 권장) |
